...
export class HolochainApp extends LitElement {
  ...
  async firstUpdated() {
    ...
    const appInfo = await appWebsocket.appInfo({
      installed_app_id: 'rea-playspace',
    });

    const cellData = appInfo.cell_data.find(data => data.role_id === 'rea_playspace') as InstalledCell;
  ...
  render() {
    return html`
      ...
      ${this.entryHash
        ? html`<span
            >Created new Holochain entry! project with hash: ${this.entryHash}</span
          >`
